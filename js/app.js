...
function renderWindowSelectionMenu() {
      ...
      <main class="flex-1 p-5 flex flex-col gap-4">
        ...
        <button id="menu-produccion"
          ...
          <div class="flex-1 min-w-0">
            ...
            <p class="text-sm text-gray-400 mt-0.5">Registrar tapas producidas hoy</p>
          </div>
          <svg class="w-5 h-5 text-gray-300 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7"/>
          </svg>
  ...
  $('menu-produccion').addEventListener('click', () => renderDailyProductionScreen());
}
...
const PRODUCTION_COLORS = [
  ...
  { value: 'transparente', label: 'Transparente', dot: '#e0f2fe', border: '#7dd3fc' },
...
async function renderDailyProductionScreen() {
  ...
  $('app').innerHTML = `
    <div class="min-h-screen bg-gray-50 flex flex-col">
      <header class="bg-white border-b border-gray-100 px-4 py-3 flex items-center gap-3 sticky top-0 z-10 shadow-sm">
        <button id="back-btn" class="w-8 h-8 flex items-center justify-center rounded-lg bg-gray-100 text-gray-500 hover:bg-gray-200 transition-colors flex-shrink-0">←</button>
        <h1 class="text-base font-bold text-gray-800">Registro de Producción</h1>
      </header>
      ...
      <main class="flex-1 p-4 space-y-4">
        ...
        <div class="bg-white rounded-2xl border border-gray-100 shadow-sm px-4 py-3">
          <p class="text-xs text-gray-400 uppercase font-semibold tracking-wide mb-0.5">Operario</p>
          <p class="text-base font-bold text-gray-800">${App.user?.name}</p>
        </div>

        <!-- Formulario de entrada -->
        <div class="bg-white rounded-2xl border border-gray-100 shadow-sm p-4">
          <p class="text-sm font-bold text-gray-700 mb-4">Nueva entrada</p>

          <!-- Fecha -->
          <div class="mb-4">
            <label class="block text-xs text-gray-500 font-semibold uppercase tracking-wide mb-1">Fecha</label>
            <input id="prod-date" type="date" value="${getCurrentDate()}"
              class="w-full border border-gray-200 rounded-xl px-3 py-2.5 text-sm text-gray-800 focus:outline-none focus:border-blue-400 transition-colors" />
          </div>

          <!-- Selector de color -->
          <div class="mb-4">
            <label class="block text-xs text-gray-500 font-semibold uppercase tracking-wide mb-2">Color de tapa</label>
            <div class="grid grid-cols-3 gap-2" id="color-grid">
              ${PRODUCTION_COLORS.map(c => `
                <button type="button" data-color="${c.value}"
                  class="color-chip flex items-center gap-2 border-2 border-gray-200 rounded-xl px-3 py-2 text-sm font-medium text-gray-700 hover:border-blue-300 active:scale-95 transition-all">
                  <span class="w-4 h-4 rounded-full flex-shrink-0" style="background:${c.dot};${c.border ? `border:1.5px solid ${c.border};` : ''}"></span>
                  <span>${c.label}</span>
                </button>
              `).join('')}
            </div>
            <p id="color-error" class="text-red-500 text-xs mt-1 hidden">Selecciona un color</p>
          </div>

          <!-- Cantidad -->
          <div class="mb-4">
            <label class="block text-xs text-gray-500 font-semibold uppercase tracking-wide mb-1">Cantidad de tapas</label>
            <input id="prod-qty" type="number" inputmode="numeric" min="1" placeholder="Ej: 500"
              class="w-full border border-gray-200 rounded-xl px-3 py-2.5 text-sm text-gray-800 focus:outline-none focus:border-blue-400 transition-colors" />
            <p id="qty-error" class="text-red-500 text-xs mt-1 hidden">Ingresa una cantidad válida</p>
          </div>

          <!-- Notas opcionales -->
          <div class="mb-5">
            <label class="block text-xs text-gray-500 font-semibold uppercase tracking-wide mb-1">Notas (opcional)</label>
            <textarea id="prod-notes" rows="2" placeholder="Observaciones..."
              class="w-full border border-gray-200 rounded-xl px-3 py-2.5 text-sm text-gray-800 focus:outline-none focus:border-blue-400 transition-colors resize-none"></textarea>
          </div>

          <button id="prod-submit"
            class="w-full bg-green-600 text-white font-bold py-3.5 rounded-xl text-sm hover:bg-green-700 active:scale-95 transition-all">
            Registrar entrada
          </button>
        </div>

        <!-- Entradas de hoy -->
        <div class="bg-white rounded-2xl border border-gray-100 shadow-sm p-4 pb-8">
          <p class="text-sm font-bold text-gray-700 mb-3">Mis entradas de hoy</p>
  ...
  document.querySelectorAll('.color-chip').forEach(chip => {
    chip.addEventListener('click', () => {
      ...
      chip.classList.add('border-blue-500', 'bg-blue-50');
  ...
  loadTodayEntries(getCurrentDate());
}
...
async function handleDailyProductionSubmit() {
  ...
  if (!qty || qty < 1 || isNaN(qty)) {
    $('qty-error')?.classList.remove('hidden');
    valid = false;
  } else {
    $('qty-error')?.classList.add('hidden');
  }
  if (!valid) return;
  ...
  try {
    ...
    showToast(navigator.onLine
      ...
      : 'Guardado sin conexión — se sincronizará al volver el internet',
      'success'
    );

    loadTodayEntries($('prod-date')?.value || getCurrentDate());

  } catch (err) {
    showToast('Error al registrar la entrada', 'error');
    console.error('handleDailyProductionSubmit:', err);
  } finally {
    btn.disabled    = false;
...
async function loadTodayEntries(date) {
  ...
  if (!navigator.onLine) {
    container.innerHTML = `<p class="text-gray-400 text-sm text-center py-4">Sin conexión — historial no disponible.</p>`;
    return;
  }

  try {
    const { data, error } = await supabaseRequest(
      `daily_production_logs?operator_id=eq.${App.user.id}&production_date=eq.${date}&select=color,quantity,status,created_at&order=created_at.asc`
    );

    if (error || !data || data.length === 0) {
      container.innerHTML = `<p class="text-gray-400 text-sm text-center py-4">Sin entradas para esta fecha.</p>`;
      return;
    }

    const colorLabel = v => PRODUCTION_COLORS.find(c => c.value === v)?.label || v;
    const colorDot   = v => {
      const col = PRODUCTION_COLORS.find(c => c.value === v);
      return col ? `background:${col.dot};${col.border ? `border:1.5px solid ${col.border};` : ''}` : 'background:#ccc;';
    };
    const statusBadge = s => s === 'confirmed'
      ? '<span class="text-xs font-semibold text-green-700 bg-green-100 px-2 py-0.5 rounded-full">Confirmado</span>'
      : '<span class="text-xs font-semibold text-yellow-700 bg-yellow-100 px-2 py-0.5 rounded-full">Pendiente</span>';

    const total = data.reduce((s, e) => s + Number(e.quantity), 0);

    container.innerHTML = `
      <div class="space-y-2">
        ${data.map(e => `
          <div class="flex items-center gap-3 py-2.5 border-b border-gray-50 last:border-0">
            <span class="w-5 h-5 rounded-full flex-shrink-0" style="${colorDot(e.color)}"></span>
            <div class="flex-1 min-w-0">
              <p class="text-sm font-semibold text-gray-800">${colorLabel(e.color)}</p>
              <p class="text-xs text-gray-400">${Number(e.quantity).toLocaleString('es-DO')} tapas</p>
            </div>
      ...
      <div class="mt-3 pt-3 border-t border-gray-100 flex justify-between items-center">
        ...
        <p class="text-sm font-bold text-gray-800">${total.toLocaleString('es-DO')} tapas</p>
      </div>
    `;
  } catch (err) {
    container.innerHTML = `<p class="text-gray-400 text-sm text-center py-4">Error al cargar entradas.</p>`;
    console.error('loadTodayEntries:', err);
  }
}
